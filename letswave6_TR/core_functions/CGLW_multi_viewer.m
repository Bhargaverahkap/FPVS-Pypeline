...
%% fig_topo_Update
    function fig_topo_Update()
        if userdata.is_topo
            ...
            [xq,yq] = meshgrid(linspace(-0.5,0.5,67),linspace(-0.75,0.75,67)); %editTR
            for dataset_index=1:length(userdata.selected_datasets)
                if(ax_idx>ax_num)
                    break;
                end
...
%% fig_topo_popup
    function fig_topo_popup(obj,~)
        if strcmp(get(gcf,'SelectionType'),'open')
            ...
            [xq,yq] = meshgrid(linspace(-0.5,0.5,267),linspace(-0.75,0.75,267)); %editTR
            delta = (xq(2)-xq(1))/2;
            ...
            for ax_idx=1:ax_num
                ...
                set(axes_topo(ax_idx),'Xlim',[-0.625,0.625]); %editTR
                %set(axes_topo(ax_idx),'Ylim',[-0.5,0.6]);
                set(axes_topo(ax_idx),'Ylim',[-0.65,0.6]); %editTR
        ...
        if userdata.is_topo
            ...
            for ax_idx=1:ax_num
                if length(handles.axes_topo)<ax_idx
                    ...
                    set(handles.axes_topo(ax_idx),'Xlim',[-0.625,0.625]); %editTR
                    %set(handles.axes_topo(ax_idx),'Ylim',[-0.5,0.6]);
                    set(handles.axes_topo(ax_idx),'Ylim',[-0.65,0.6]); %editTR
