function [ST] = Basis_regressors(EEG,ST)
ST.regressor=[];
if ST.method_type==5  
    T=find(ST.f_T*1000>=ST.interval(1)&ST.f_T*1000<=ST.interval(2));
    dat=squeeze(mean(ST.f_trials(T,:),2));
    for i=1:size(ST.peak,1)
        T_peak(i)=find(ST.f_T*1000==ST.peak(i,1))-T(i)+1;    
    end
    t=ST.f_T(T);
else
    T=find(EEG.times>=ST.interval(1)&EEG.times<=ST.interval(2));
    dat=squeeze(mean(EEG.data(ST.channel_index,T,:),3));
    for i=1:size(ST.peak,1)
        T_peak(i)=find(EEG.times==ST.peak(i,1))-T(i)+1;    
    end
    t=EEG.times(T);
end
...
if Fs>500
    index_=3;%% if out of memory, this number should be increased
elseif Fs<=500
    index_=1; 
end
