...
% Last Modified by GUIDE v2.5 19-Jun-2013 15:35:22
...
for filepos=1:length(inputfiles);
    ...
    outheader.history(i).description='GLW_align_recording_blocks';
...
%         plot(dxstart,mean(squeeze(data(1,3,1,1,1,dxstart-10:dxstart+10))),'ro');
            ...
            for indexpos=1:size(data,3);
                for dz=1:size(data,4);
                    for dy=1:size(data,5);
                        outdata(epochpos,chanpos,indexpos,dz,dy,dxstart:dxend) = data(epochpos,chanpos,indexpos,dz,dy,dxstart:dxend) + voltDiff(epochpos,chanpos,indexpos,dz,dy,:);
                    end
                end
            end
        end        
        %store last voltage of current block
        previousBlockVolt = outdata(epochpos,:,:,:,:,dxend);        
    end    
    ...
    %save header
    [p,n,e]=fileparts(inputfiles{filepos});
    st=[p,filesep,get(handles.prefixtext,'String'),' ',n,'.lw6'];
    disp(['saving ',st]);
    save(st,'-MAT','header');
    %save data
    [p,n,e]=fileparts(inputfiles{filepos});
