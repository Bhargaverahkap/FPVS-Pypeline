...
flag_assign_spline = 0;
flag_build_spline = 0;
flag_bio_to_1020 = 0;
flag_1020_to_bio = 0;
if get(handles.checkbox_assign_location,'value') flag_assign_location = 1; end
if get(handles.checkbox_assign_spline,'value') flag_assign_spline = 1; end
if get(handles.checkbox_build_spline,'value') flag_build_spline = 1; end
...
for filepos=1:length(inputfiles);
    ...
    if(flag_assign_spline)
        %splinefile name
        [ps,ns,es]=fileparts(get(handles.splinefile,'String'));
        splinefilename=[ns,'.spl'];
        %copy splinefile in current directory if it does not exist
        if exist([p,filesep,splinefilename])==0;
            disp(['Copying splinefile to : ',p,filesep,splinefilename]);
            copyfile([directory_spherical_location, get(handles.splinefile,'String')],[p,filesep,splinefilename]);
        end;
        %assign
        disp('*** Assigning spline file');
        [header]=LW_assignsplinefile(header,splinefilename);
    end
    
    if(flag_build_spline)
        splinefilename=[p,filesep,n,'.spl'];
        disp('*** Building splinefile (this may take a while!)');
