function varargout = GLW_multiview(varargin)
% GLW_MULTIVIEW MATLAB code for GLW_multiview.fig
%
% Author : 
% Andr? Mouraux
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GLW_multiview_OpeningFcn, ...
                   'gui_OutputFcn',  @GLW_multiview_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
for graphcolpos=1:numgraphcols;
    for graphrowpos=1:numgraphrows;
        ...
        for wavepos=1:numwaves;
            if display_waves==1;
                hh=plot(tpx(graphrowpos,graphcolpos,wavepos).data,squeeze(tpdata(graphrowpos,graphcolpos,wavepos,1:length(tpx(graphrowpos,graphcolpos,wavepos).data))),'Color',lcolors(wavepos,[1,2,3]));
%             set(hh,'marker','o','markersize',5);
            end;
            if display_waves==2;
                hh=stem(tpx(graphrowpos,graphcolpos,wavepos).data,squeeze(tpdata(graphrowpos,graphcolpos,wavepos,1:length(tpx(graphrowpos,graphcolpos,wavepos).data))),'Color',lcolors(wavepos,[1,2,3]));               
%                 hbase = get(hh,'Baseline');
%                 set(hbase,'BaseValue',1);
%                 set(hh,'marker','o','markersize',5);
            end;
            if display_waves==3;
...
% --- Executes just before GLW_multiview is made visible.
function GLW_multiview_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GLW_multiview (see VARARGIN)
% Choose default command line output for GLW_multiview
...
for filepos=1:length(inputfiles);
    ...
    disp(['Loading : ',st]);
    load(st,'-MAT');
    st=[get(handles.inputdir,'String'),filesep,n,'.mat'];
    load(st,'-MAT');
...
function varargout = GLW_multiview_OutputFcn(hObject, eventdata, handles) 
...
for filepos=1:length(files);
    ...
    for epochpos=1:length(epochs);
        if get(handles.scalppopup,'Value') == 3 % take the mean in a window- added by CJ 15/06/2013
            meanWinData = userdata(files(filepos)).data(epochs(epochpos),:,index,dz,dy,dcursor1:dcursor2);
            meanWinData(:,:,:,:,:,1:2) = [mean(meanWinData,6) ;mean(meanWinData,6)]';
            meanWinData(:,:,:,:,:,3:end) = [];
            tpaxis=LW_topoplot(header,meanWinData,epochs(epochpos),index,1,dy,dz,'maplimits',maplimits,'shading','interp','whitebk','on','electrodes','on');
        else
            ...
            tpaxis=LW_topoplot(header,userdata(files(filepos)).data,epochs(epochpos),index,dx,dy,dz,'maplimits',maplimits,'shading','interp','whitebk','on','electrodes','on');
        end
...
for filepos=1:length(files);
    ...
    for epochpos=1:length(epochs);
        if get(handles.scalppopup,'Value') == 3 % take the mean in a window- added by CJ 15/06/2013
            ...
            meanWinData(:,:,:,:,:,1:2) = [mean(meanWinData,6) ;mean(meanWinData,6)]';
            meanWinData(:,:,:,:,:,3:end) = [];           
            LW_headplot(header,meanWinData,epochs(epochpos),index,1,dy,dz,'maplimits',[ycmin ycmax],'electrodes','on');
        else
            %find maximum and minimum
            tp=squeeze(userdata(files(filepos)).data(epochs(epochpos),channels(1),index,dz,dy,dcursor1:dcursor2));
            [all_maxy,all_maxi]=max(tp);
            ...
            tpaxis=LW_headplot(header,userdata(files(filepos)).data,epochs(epochpos),index,dx,dy,dz,'maplimits',[ycmin ycmax],'electrodes','on');%,'cbar',0
        end
