...
for filepos=1:length(inputfiles);
    ...
    %-- 1 save CI
    %     a. low CI
    data = zeros(1,size(ci,2),1,1,1,size(ci,6));
    data(1,:,1,1,1,:) = ci(1,:,:,:,:,:);
    %save header   
    prefix = [preprefix,' ciLow_',num2str(1-percentCI)];
    exte = '.lw5';
    st=[p,filesep,prefix,' ',n,exte];
    disp(['saving ',st]);
    save(st,'-MAT','header');
    %save data
    exte = '.mat';
    ...
    if get(handles.checkbox_sigMask,'Value');
        isPositive = zeros(size(PvalueStore));
        isNegative = zeros(size(PvalueStore));
        ...
        % build mask
        data = zeros(1,size(PvalueStore,1),1,1,1,size(PvalueStore,5));
