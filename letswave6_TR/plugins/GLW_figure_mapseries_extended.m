...
%     load(st,'-mat');
    ...
    for i=1:length(dxi);
        ...
        if get(handles.radiobutton_scalpMap,'Value')
            LW_topoplot(header,dataCell{rowpos},epochpos,index,dxi(i),dy,dz,'maplimits',[cmin cmax],'shading','interp','whitebk','on','electrodes',electrodeOn);
        %colorbar; %editTR to turn off color bar
        end
        %colormap
