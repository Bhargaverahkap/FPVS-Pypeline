function [out_configuration,out_datasets] = LW_sum_epochs(operation,configuration,datasets,update_pointers)
% LW_sum_epochs
% Sum epochs
%
% operations : 
...
%gui_info
gui_info.function_name='LW_sum_epochs';
gui_info.name='Sum epochs';
gui_info.description='Sum epochs across trials.';
gui_info.parent='sum_menu';
...
gui_info.process_filename_string='sum';         %default filename suffix
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.operation='sum';
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Sum epochs.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            ...
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_sum_epochs(datasets(setpos).header,datasets(setpos).data,'operation',configuration.parameters.operation);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_average_epochs('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
