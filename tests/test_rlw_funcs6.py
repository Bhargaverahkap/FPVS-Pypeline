...
only0, _ = rlw.ica_remix_data(comps, m, mm, ic_list=[0])
...
matrix = rlw.pca_compute_data(data)
assert np.allclose(matrix["ica_mm"] @ matrix["ica_um"], np.eye(2), atol=1e-8)
comps, _, _ = rlw.ica_unmix_data(data, meta, matrix["ica_um"])
...
out, _ = rlw.ocular_remove_data(d, emeta, ["EOG"])
...
out, _ = rlw.suppress_artifact_data(spike, meta_for(["A"]), x_start=8/fs, x_end=13/fs)
...
out, _ = rlw.suppress_artifact_event_data(dirty, meta_for(["A"], ev), "stim",
...
sev = {"code": np.array(["10", "10"], dtype=object), "latency": np.array([1.0, 4.0]),
       "epoch": np.array([0, 0])}
out, m = rlw.segmentation_ssep_data(long, meta_for(["A"], sev), ["10"],
...
m = rlw.edit_electrodes_seeg_data(meta_for(["A", "B"]), ["B"], [1], [2], [3])
