...
out, m = rlw.reject_epochs_data(data, meta, [1])
...
out, m = rlw.reject_epochs_amplitude_data(loud, meta, criterion=500)
...
out, _ = rlw.reject_epochs_amplitude_data(partly, meta, 500, selected_channel_labels=["Fp1"])
assert out.shape[2] == 3, out.shape
out, _ = rlw.reject_epochs_amplitude_data(partly, meta, 500, selected_channel_labels=["Cz"])
...
out, _ = rlw.reject_epochs_amplitude_data(spike, meta, 500, x_limits=True, x_start=1/fs)
assert out.shape[2] == 3, out.shape
ok.append("reject_epochs_amplitude")

# concatenate_epochs: 3 epochs of 4 samples -> one epoch of 12, latencies shifted
out, m = rlw.concatenate_epochs_data(data, meta)
...
out = rlw.equalize_epochs_data([(big, meta), (small, meta)])
assert [d.shape[2] for d, _ in out] == [2, 2]
out = rlw.equalize_epochs_data([(big, meta)], num_epochs=3, random_selection=True, seed=1)
assert out[0][0].shape[2] == 3
ok.append("equalize_epochs")

# index axis
four = np.zeros((4, 3, 3, 5))
out, m = rlw.arrange_index_data(four, dict(meta, index_labels=[f"i{i}" for i in range(5)]), [4, 0])
assert out.shape == (4, 3, 3, 2) and m["index_labels"] == ["i4", "i0"]
out, _ = rlw.arrange_index_data(data, meta, [0])         # 3-D: no-op
assert out.shape == data.shape
merged, m = rlw.merge_index_data([(data, meta), (data, meta)])
...
out, m = rlw.select_epochdata_data(data, emeta, "rt", "<", 0.3)
assert out.shape[2] == 1 and np.allclose(out[:, :, 0], data[:, :, 1])
out, m = rlw.sort_epochdata_data(data, emeta, "rt")
assert out.shape[2] == 2
assert np.allclose(out[:, :, 0], data[:, :, 1]), "0.1 should sort first"
out, _ = rlw.sort_epochdata_data(data, emeta, "rt", "descend", discard_empty=False)
