...
out, m = rlw.merge_channels_data([(data, meta), (data[:, :2] * 10, other)])
...
# pool_channels
out, m = rlw.pool_channels_data(data, meta, ["C3", "C4"])
assert out.shape == (6, 5, 2)
assert np.allclose(out[:, 4], (data[:, 0] + data[:, 1]) / 2)
assert rlw.channel_labels(m)[-1] == "newchan"
out, m = rlw.pool_channels_data(data, meta, ["C3","C4"], channel_weights=[3, 1],
...
# linear_channel_map
out, m = rlw.linear_channel_map_data(data, meta, num_lines=7)
...
w, labels = rlw.weighted_channel_average_template_data(peak, meta, x=3/fs, num_channels=2)
assert labels == ["C4", "Pz"], labels
assert np.allclose(w, [4/7, 3/7]), w
out, m = rlw.weighted_channel_average_apply_data(peak, meta, w, labels)
