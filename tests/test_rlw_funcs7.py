...
wdata = trials[:, None, :]
wmeta = meta_for(["Cz"])
mask, mmeta = rlw.wavelet_filter_build_data(wdata, wmeta, "Cz", 5, 20, 5, threshold=0.8)
assert mask.shape == (4, n), mask.shape
assert abs(mask.mean() - 0.2) < 0.02, mask.mean()
out, om = rlw.wavelet_filter_apply_data(wdata, wmeta, mask, mmeta, "Cz")
...
out, m = rlw.pan_tompkin_data(d, emeta, "EK1")
