...
out, m = rlw.stfft_data(data, meta, hanning_width=0.25, sliding_step=8,
...
out, m = rlw.stfft_zhang_data(data, meta, hanning_width=0.25, low_frequency=1,
...
out, m = rlw.cwt_data(data, meta, 1, 30, 30, average_epochs=False)
power = np.abs(out[:, :, 0, 0])
assert abs(m["freqs"][np.argmax(power.max(axis=0))] - 10) < 2
assert power[-n//3:].max() > 3 * power[:n//4].max()
ok.append("cwt")

out, m = rlw.cwt_fast_data(data, meta, 5, 20, 8, mother_size=800, average_epochs=False)
power = out[:, :, 0, 0]
...
out, _ = rlw.hilbert_data(am, meta)
assert np.corrcoef(out[50:-50, 0], env[50:-50])[0, 1] > 0.95
ok.append("hilbert")

out, m = rlw.hilbert_bands_data(am, meta, freq_start=30, freq_end=50, freq_lines=5, freq_width=8)
...
out, _ = rlw.stfft_data(two, meta, num_frequency_lines=10, average_epochs=True)
