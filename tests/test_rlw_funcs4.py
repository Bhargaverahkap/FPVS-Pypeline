...
out, m = rlw.select_events(data, meta, "go", 0, 0.6)
assert out.shape[2] == 1 and np.allclose(out[:, :, 0], data[:, :, 0])
out, _ = rlw.select_events(data, meta, "go", 0, 1.0)
assert out.shape[2] == 2
out, _ = rlw.select_events(data, meta, "nope")
...
out, m = rlw.sort_events(data, meta, "go", "descend")
assert out.shape[2] == 2 and np.allclose(out[:, :, 0], data[:, :, 1])
out, _ = rlw.sort_events(data, meta, "go", "ascend", discard_empty=False)
...
m = rlw.events_delete_duplicate(dmeta)
assert len(rlw.events_to_list(m)) == 3, rlw.events_to_list(m)   # exact dup only
m = rlw.events_delete_duplicate(dmeta, exact_latencies=False, tolerance=0.1)
...
m = rlw.events_level_trigger(trig, tmeta, "Trig", threshold=1000, min_isi=0.1)
...
out, _ = rlw.average_epochs_sliding(ramp, meta, "average", width=3/fs)
assert np.isclose(out[5, 0, 0], 5.0), out[5, 0, 0]        # mean(3..7)
assert np.isclose(out[0, 0, 0], 1.0), out[0, 0, 0]        # window clipped to 0..2
out, _ = rlw.average_epochs_sliding(ramp, meta, "max", width=3/fs)
...
out, m = rlw.average_erpimage(many, meta, num_lines=8, smooth=False)
assert out.shape == (20, 8, 2, 1), out.shape
assert np.allclose(out[:, 3, :, 0], many[:, :, 3])
out, m = rlw.average_erpimage(many, meta, num_lines=5, smooth=True, smooth_width=3)
assert out.shape == (20, 5, 2, 1), out.shape
ok.append("average_erpimage")

# grand_average
a = np.ones((10, 2, 4)); b = np.full((10, 2, 1), 3.0)
out, _ = rlw.grand_average([(a, meta), (b, meta)])
assert out.shape == (10, 2, 1) and np.allclose(out, 2.0), out[0, 0]
out, _ = rlw.grand_average([(a, meta), (b, meta)], [3, 1])
assert np.allclose(out, 1.5), out[0, 0]
ok.append("grand_average")

# properties
m = rlw.properties(meta, filetype="frequency_amplitude", xstart=-2, xstep=0.5)
