...
# crop
out, m = rlw.crop(data, meta, x_start=0.5, x_size=100)
...
# dc_removal
out, _ = rlw.dc_removal(data, meta)
assert np.allclose(out.mean(axis=0), 0, atol=1e-12)
out, _ = rlw.dc_removal(np.arange(n)[:, None] * 1.0, meta, linear_detrend=True)
...
out, m = rlw.ifft(spec, smeta, time_meta_data=meta)
...
out, _ = rlw.fft_filter(tone, meta, "bandpass", 5, 15, 0, 0)
...
out, _ = rlw.fft_filter(tone, meta, "notch", 55, 65, 0, 0)
resid = np.abs(np.fft.rfft(out[:, 0]))
...
# resample
out, m = rlw.resample(data, meta, x_sampling_rate=128)
